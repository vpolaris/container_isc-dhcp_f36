...
RUN dnf -y --installroot=${sysroot} ${DNFOPTION} --releasever ${DISTVERSION} install glibc.${arch} setup shadow-utils
...
#dhcpd prerequisites
RUN dnf -y --installroot=${sysroot} ${DNFOPTION} --releasever ${DISTVERSION} install coreutils cyrus-sasl-lib keyutils-libs krb5-libs libcap libcom_err libevent libgcrypt libgpg-error libstdc++ libzstd lz4-libs p11-kit openssl-libs openldap procps-ng util-linux xz-libs zlib
RUN dnf -y --installroot=${sysroot} ${DNFOPTION} --releasever ${DISTVERSION} install --downloadonly --downloaddir=./ gnutls.${arch} dhcp-server.${arch} initscripts
...
#install dhcpd
RUN DHCPRPM="$(ls dhcp-server*${arch}.rpm)" \
...
RUN INITRPM="$(ls initscripts*${arch}.rpm)" \
